 ...
 (function(f) {
     ...
     return (function e(t, n, r) {
         ...
         3: [function(_dereq_, module, exports) {
             ...
             // webkit提供一个非常有用的textInput事件，被直接用来代替beforeInput
             // ie的这个事件没有鸟用，忽略
             // Opera <= 12 不会触发
             ...
             // 是否应该使用回退的合成数据
             // canUseTextInputEvent为假 或 8<ie<=11
             ...
             // 跟踪是否在keypress时候有space键按下
             // Track whether we've ever handled a keypress on the space key.
             var hasSpaceKeypress = false;

             // 是否在keypress事件被认为一个快捷键命令命令
             /**
             ...
             function isKeypressCommand(nativeEvent) {
                // ctrl或alt或meta键被按下，但ctrl和alt没有同时按下
         ...
         4: [function(_dereq_, module, exports) {
              ...
              */
             // prefixKey 给key加上prefix前缀并将key首字母大写
             ...
             // for-in循环会让ie8陷入无限循环，因为它会在每次遍历时候加上新的属性
             // 给 isUnitlessNumber 的个属性加上各厂商前缀属性
              ...
              * special prodding.
              */
             // ie8不支持对以下样式属性使用简记属性 如backgroundColor不能写成background-color
         ...
         // {CSSPropertyOperations} 处理css属性的操作
         5: [function(_dereq_, module, exports) {
             ...
             // 测量CSSPropertyOperations的setValueForStyles方法性能
         ...
         // CallbackQueue 跟踪组件到呈现的dom可用，触发回掉函数队列，最后清空队列
         ...
         // {ClientReactRootIndex} createReactRootIndex方法获取下一个react标识
         ...
         // {DOMChildrenOperations} dom孩子节点操作
         9: [function(_dereq_, module, exports) {
             ...
             var DOMChildrenOperations = {
                 ...
                 // 通过处理一系列的更新来更新组件的孩子，每个更新配置应该有parentNode属性
                 /**
         ...
         // {DOMProperty} properties：保存各个dom属性的信息
         ...
         // {DOMPropertyOperations} 处理DOMProperty
         ...
         // {Danger} dangerouslyRenderMarkup：将包含标记数组转化为含节点的数组  dangerouslyReplaceNodeWithMarkup：将节点使用标记进行替换
         ...
         // [DefaultEventPluginOrder] 默认的被注入 EventPluginHub 的事件插件顺序
         ...
         // {EventPluginHub} 对事件插件进行安装和配置的统一接口
         16: [function(_dereq_, module, exports) {
             ...
             // 模拟 executeDispatchesAndRelease
             var executeDispatchesAndReleaseSimulated = function(e) {
                 return executeDispatchesAndRelease(e, true);
             };
             // 非模拟 executeDispatchesAndRelease
             var executeDispatchesAndReleaseTopLevel = function(e) {
                 return executeDispatchesAndRelease(e, false);
             };

             // 用于执行给定逻辑dom元素的ids相应的dom层次逻辑遍历的需要的模块
             /**
             ...
             var EventPluginHub = {
                 ...
                 // 存储监听器到listenerBank[registrationName][id]中，是幂等的
                 /**
                 ...
                 deleteListener: function(id, registrationName) {
                     var PluginModule = EventPluginRegistry.registrationNameModules[registrationName];
                     // 模块存在且有willDeleteListener方法，则调用
                     if (PluginModule && PluginModule.willDeleteListener) {
                         PluginModule.willDeleteListener(id, registrationName);
                     }

                     // 删除listenerBank[registrationName]中的对应监听器
                     var bankForRegistrationName = listenerBank[registrationName];
         ...
         // {EventPluginRegistry} 注册模块，可以提取和分发事件
         ...
         18: [function(_dereq_, module, exports) {
             ...
             var injection = {
                 ...
                 injectMount: function(InjectedMount) {
                     ...
                     if ("development" !== 'production') {
                         // 上面的判断感觉像是多余的圈圈
                         // 被注入的InjectedMount需要存在且有getNode和getID方法，否则发出警告
             ...
             if ("development" !== 'production') {
                 validateEventDispatches = function(event) {
                     ...
                     // listeners和ids同时为数组或同时不为，并且长度相等
             ...
             // 按顺序遍历监听器并执行，返回第一个返回true的domId，没有则返回null
             /**
             ...
             // 执行executeDispatchesInOrderStopAtTrueImpl后做一些清理工作
             /**
         ...
         // FallbackCompositionState 这个帮助类储存目标结点的文本信息，允许在事件发生前后进行内容比较
         ...
         // {HTMLDOMPropertyConfig} html dom的属性配置
         ...
         // {PooledClass} 实例池，有则从池子中取并初始化，没有则实例化
         24: [function(_dereq_, module, exports) {
             ...
             // 若池子中有实例则取出一个进行重新初始化，若没有则实例化一个 最后将实例返回回来
             /**
         ...
         // {ReactBrowserComponentMixin} 浏览器合成mixin
         ...
         // {ReactBrowserEventEmitter} 事件处理汇总
         ...
         // ReactComponentEnvironment 组件管理环境注入
         ...
         // {ReactCurrentOwner} React组件的当前所属
         ...
         // ReactDOMButton getNativeProps：获取本地属性(非鼠标监听事件属性)
         ...
         // ReactDOMFeatureFlags useCreateElement是否需要创建元素
         ...
         // {ReactDOMIDOperations} dom节点id更新操作
         ...
         // {ReactDefaultBatchingStrategy} 默认的批量更新策略
         ...
         // ReactDefaultPerfAnalysis React默认性能分析
         ...
         52: [function(_dereq_, module, exports) {
             ...
             // Symbol备用来做ReactElement的类型标记，若没有且没有垫片，则为了性能而使用一个简洁的数字来代替
             ...
             // 对所有react elements的基本的构造器 这仅仅被用来执行动态的实例检测
             /**
         ...
         // {ReactEmptyComponentRegistry} React空组件管理
         ...
         // ReactInstanceHandles React实例句柄， 操作 指派给的代表react组件dom的 id，使用这种特别的模式来高效便利dom
         61: [function(_dereq_, module, exports) {
             ...
             function isValidID(id) {
                 // 为空字串 或 位置0是分隔符且末字符不是分隔符
                 return id === '' || id.charAt(0) === SEPARATOR && id.charAt(id.length - 1) !== SEPARATOR;
             }

             // 校验是否第一个id是第二个id的祖先或相等
             /**
             ...
             // 在两个id间遍历父路径 两个id必须不同 必须是父子关系 如果回掉函数返回false就停止遍历
             /**
             ...
             var ReactInstanceHandles = {
                 ...
                 // 获取react组件的domId，是包含带有所给domId的组件的树的根
                 /**
                 ...
                 getReactRootIDFromNodeID: function(id) {
                     ...
                     if (id && id.charAt(0) === SEPARATOR && id.length > 1) {
                         ...
                         // 有第二个分隔符就截取到第二个，没有则返回本身
                         return index > -1 ? id.substr(0, index) : id;
                     }
                     return null;
                 },

                 // 遍历id层次并且在任何可能受到鼠标进入和离开事件的id上执行所给回掉
                 /**
                 ...
                 // 同traverseTwoPhase，不过要跳过targetID
                 /**
         ...
         65: [function(_dereq_, module, exports) {
             ...
             // 被用来在findComponentRoot时候存储广度优先搜索的状态
             ...
             // 若合法则是被当前挂载的container包含
             // 这意味着节点没有被包含在文档中才被认为是合法的
             // id>=node
             /**
             ...
             function findDeepestCachedAncestorImpl(ancestorID) {
                 ...
                 // 节点存在，且ancestorID>=ancestor
                 if (ancestor && isValid(ancestor, ancestorID)) {
                     deepestNodeSoFar = ancestor;
                 } else {
                    // 节点不在缓存中，所以推测他没有后代了，退出循环
         ...
         // {ReactMultiChildUpdateTypes} 枚举所有可能的更新配置类型
         67: [function(_dereq_, module, exports) {
             ...
             // 当一个组件的孩子更新后，一系列的更新配置对象被创建来决定哪些变化了
             /**
         ...
         // {ReactNativeComponent} React本地组件
         ...
         // ReactNoopUpdateQueue 更新队列的抽象api
         69: [function(_dereq_, module, exports) {
             ...
             var ReactNoopUpdateQueue = {
                 ...
                 // 替换所有状态 使用此方法或setState方法来改变状态，把this.state看作是不变的
                 // 不保证this.state会立即更新
                 /**
         ...
         // {ReactPerf} React通用的AOP系统，被设计来测量性能
         ...
         // {ReactPropTypeLocationNames} React属性类型标记名
         ...
         // {ReactPropTypeLocations} React属性类型标记
         ...
         // {ReactRootIndexInjection} 创建React根索引
         ...
         // ReactServerBatchingStrategy ReactServer批处理策略
         ...
         // ReactServerRenderingTransaction 提供一个CallbackQueue队列来收集在transaction执行期间onDOMReady的回掉
         ...
         82: [function(_dereq_, module, exports) {
             ...
             function getInternalInstanceReadyForUpdate(publicInstance, callerName) {
                 ...
                 if ("development" !== 'production') {
                     // 当前有ReactCurrentOwner.current就警告
         ...
         // {SVGDOMPropertyConfig} svg的dom属性配置
         ...
         // {ServerReactRootIndex} createReactRootIndex创建react唯一标识
         ...
         // SyntheticClipboardEvent 合成剪贴板事件
         ...
         // SyntheticCompositionEvent 合成组合事件
         ...
         // Transaction 包裹任何方法，在方法执行前后把不变的保存起来,即使发生异常
         100: [function(_dereq_, module, exports) {
             ...
             var Mixin = {
                 ...
                 // 初始化从startIndex开始到结束的所有过渡wrappers
         ...
         // accumulateInto 将俩个对象合并成一个数组，若第一个为数组，则会合并到第一个里面去
         ...
         // canDefineProperty 判断是否能够定义对象属性
         ...
         // dangerousStyleValue 某个css属性值，若无单位就返回本身，有则加px
         ...
         // deprecated 记录个别废弃的方法，给出警告并给出新api的位置
         ...
         // escapeTextContentForBrowser 转义html特殊字符
         ...
         // forEachAccumulated 对数组中每个对象 或 单个对象 做某种操作
         ...
         // getEventCharCode 获取事件对象的键码, 若不可打印则返回0
         ...
         // getEventModifierState 将特殊键转化为关联的属性
         ...
         // 获取某个元素开始的含有偏移offset位置的文本节点信息，若没有则迭代获取父节点的下个兄弟节点继续找
         116: [function(_dereq_, module, exports) {
              ...
              * @return {?DOMElement|DOMTextNode}
              */
             // 获取一个元素的 下个兄弟节点 或 若没有则迭代获取父节点的下个兄弟节点
         ...
         // getTextContentAccessor 获取节点文本的方法名
         ...
         // onlyChild 返回集合中的第一个孩子，并核实集合里面只有一个
         ...
         // quoteAttributeValueForBrowser 将字符中的html特殊字符转义后，两边加上"
         ...
         // shouldUpdateReactComponent 判断一个元素是否需要更新
         ...
         // traverseAllChildren 遍历节点，获取叶子节点数
         ...
         // validateDOMNesting 报告不正确的嵌套标签信息
         ...
         // {EventListener} 事件监听器对象 提供捕获(capture)和冒泡(listen)阶段的事件监听
         ...
         // camelizeStyleName -开头的字串的-去掉后然后调用camelize
         ...
         // containsNode 判断第一个节点和第二个节点 是不是 包含或相等关系
         ...
         // createArrayFromMixed 探测一个对象是否为类数组或数组，若是则转化为数组，否则用数组把此对象包起来
         ...
         // createNodesFromMarkup 将标记文本转化为数组形式的节点元素列表
         ...
         // emptyFunction 空函数 返回固定值的函数，返回真假的函数，返回Null的函数，返回this的函数，返回第一个参数的函数
         ...
         // getMarkupWrap 获取一个标签是否应该被包裹以及被包裹的层数和标签
         ...
         // getUnboundedScrollPosition 获取滚动条的left,top
         ...
         // hyphenate 把驼峰命名的字串 转化为 用连字符-连接的字串
         ...
         // hyphenateStyleName 调用hyphenate方法后，把ms-开头的字串转化为-ms-
         ...
         // keyOf 获取对象自身拥有属性的第一个属性名，没有则为null
         ...
         // mapObject 对象的每个属性值对调用某个函数，返回新对象
         ...
         // memoizeStringOnly 缓存后面可能用到的接受一个字符串参数的函数值
         ...
         // warning 输出警告消息，抛出错误，以便查看触发的调用点
